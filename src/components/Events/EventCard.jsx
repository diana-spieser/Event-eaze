import { SimpleGrid, Box, Image, Text, Flex, useColorModeValue, } from '@chakra-ui/react';
...
const EventCard = ({ events }) => {
  ...
  useEffect(() => {
    setDisplayedEvents(events.slice(0, visibleEvents));
  ...
  return (
    <SimpleGrid columns={ [1, 2, 3, 4] } spacing='25px'>
      {displayedEvents.map((event) => {
        const { singleEventUrl, startDateString, city } =
          calculateEventValues(event);
        return (
          <Box position='relative' key={ event.id }>
            <RouterLink to={ singleEventUrl }>
              <Box
                borderWidth='1px'
                shadow='md'
                rounded='lg'
